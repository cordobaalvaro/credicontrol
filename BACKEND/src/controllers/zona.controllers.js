...
const obtenerZonaOverview = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, msg, data } = await obtenerZonaOverviewBD(id);
    return res.status(status).json({ msg, data });
  } catch (error) {
    return res.status(500).json({ msg: error.message, data: null });
  }
};

const obtenerClientesZona = async (req, res) => {
  try {
    const { id } = req.params;
    const { q, page, limit } = req.query;
    const { status, msg, data } = await obtenerClientesZonaBD({
      zonaId: id,
      q,
      page,
      limit,
...
const obtenerPrestamosZona = async (req, res) => {
  try {
    const { id } = req.params;
    const { q, estado, page, limit } = req.query;
    const { status, msg, data } = await obtenerPrestamosZonaBD({
      zonaId: id,
      q,
      estado,
      page,
...
const obtenerZonaPorId = async (req, res) => {
  try {
    const { q } = req.query;
    const { status, msg, data } = await obtenerZonaPorIdBD(req.params.id, q);
    res.status(status).json({ msg, data });
  } catch (error) {
    res.status(500).json({ msg: error.message, data: null });
  }
