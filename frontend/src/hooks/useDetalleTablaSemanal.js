...
export const useDetalleTablaSemanal = (tablaId) => {
    const [tablaLocal, setTablaLocal] = useState(null)
    ...
    const handleTablaActualizada = useCallback((tablaActualizada) => {
        if (tablaActualizada?.items) {
    ...
    const obtenerTablaActualizada = useCallback(async () => {
        if (!tablaId) return

        try {
            setLoading(true)
            const response = await tablaSemanalService.getTablaById(tablaId)
            if (response?.data) {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "No se pudo obtener la tabla semanal",
            })
            setError("No se pudo obtener la tabla semanal")
        } finally {
            setLoading(false)
        }
    }, [tablaId, handleTablaActualizada])
    ...
    const modificarEsperadoCuota = async (itemId, nuevoMontoEsperado, numeroCuota) => {
        try {
            const response = await tablaSemanalService.modificarEsperado(
                tablaLocal?._id,
                itemId,
                {
                ...
                if (response.data.itemActualizado) {
                    setTablaLocal((prev) => ({
                        ...
                        items: (prev.items || []).map((item) =>
                            item._id === response.data.itemActualizado._id ? response.data.itemActualizado : item,
                        ),
    ...
    const handleGuardarEdicionAdmin = async () => {
        try {
            ...
            if (itemsMontosCobrado.length > 0) {
                const promesaMontosCobrado = tablaSemanalService.actualizarMontos(
                    tablaLocal?._id,
                    itemsMontosCobrado
                )
    ...
    const handleItemAgregado = (tablaActualizada) => {
        handleTablaActualizada(tablaActualizada)
    ...
    const handleEliminarItem = async (itemId) => {
        try {
            ...
            const response = await tablaSemanalService.eliminarItem(tablaLocal?._id, itemId)
    ...
    const handleEliminarSeleccionados = async () => {
        ...
        if (result.isConfirmed) {
            try {
                const promesas = itemsSeleccionados.map((itemId) => tablaSemanalService.eliminarItem(tablaLocal?._id, itemId))
