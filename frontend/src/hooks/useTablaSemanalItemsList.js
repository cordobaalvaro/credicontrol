...
export const useTablaSemanalItemsList = ({
  ...
  const handleResetItem = async (item) => {
    try {
      setGuardandoItemId(item._id)
      setError("")
      const response = await tablaSemanalService.actualizarItemCobrador(tabla._id, item._id, 0);
      if (response?.data) {
        handleMontoChange(item._id, 0)
        onTablaActualizada?.(response.data)
      }
    } catch (err) {
      console.error("Error al resetear item", err)
      setError(err?.response?.data?.msg || "No se pudo quitar el cobro.")
    } finally {
      setGuardandoItemId(null)
    }
  }
